...
public class OfertaSQL {
    ...
    public void agregarOferta(Oferta o){
        try {
            String sql = "INSERT INTO OFERTA (CODOFERTA,PORC_DESC,INICIO,FIN,NUM_PROD,CODPROD) VALUES (?,?,?,?,?,?) ";
    ...
    public void actualizarOferta(Oferta o){
        try {
            String sql = "UPDATE OFERTA SET PORC_DESC = ?, INICIO = ?, FIN = ?, NUM_PROD = ?, CODPROD = ? WHERE CODOFERTA = ? ";
            con = conectar.getConnection();
            ps = con.prepareStatement(sql);                                 
            ...
            System.out.println("Error SQL:" + e.getMessage() + e.getErrorCode());
        }
    }
    ...
    public boolean buscarOferta(Oferta o){
        ...
        try {
            ...
            if (rs.next()) {
                ...
                o.setId_producto(Integer.parseInt(rs.getString("CODPROD")));
                return true;
            }
            return false;
        } catch (SQLException e) {
